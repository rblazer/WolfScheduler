...
public class WolfSchedulerTest {
	 ...
	 * @throws Exception if setup fails for some reason
	 */
	@Before
	...
	/**
	 * Tests event conflicts.
	 */
	@Test
	public void testAddEventConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);
		
		ws.addEventToSchedule(TITLE, EVENT_MEETING_DAYS, EVENT_START_TIME, EVENT_END_TIME, EVENT_DETAILS);
		
		try {
			ws.addEventToSchedule("Lunch", "MTWHF", 830, 930, "lunch time");
			fail();
		} catch (IllegalArgumentException e) {
	...
	/**
	 * Tests conflict between a course and an event
	 */
	@Test
	public void testAddConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);
		
		ws.addCourseToSchedule(NAME, SECTION);
		
		try {
			ws.addEventToSchedule("Lunch", "MTWHF", 1300, 1500, "lunch time");
			fail();
		} catch (IllegalArgumentException e) {
			assertEquals("The event cannot be added due to a conflict.", e.getMessage());
		}
	...
	/**
	 * Testing multiple adds of events and courses
	 */
	@Test
	public void testAddMultipleConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);
